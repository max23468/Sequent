...
import { isCodexEnabled, isDizEnabled } from "$lib/server/config";
...
export const actions = {
  ...
  importDiz: async ({ locals, params, request }) => {
    if (!locals.ownerId) redirect(303, "/login");
    if (!isDizEnabled())
    ...
    try {
      await importDiz(database, { practiceId: params.id, declarationId, file });
    } catch (flowError) {
    ...
    redirect(303, `/pratiche/${params.id}?sezione=official&dichiarazione=${declarationId}`);
  },
  exportDiz: async ({ locals, params, request }) => {
    if (!locals.ownerId) redirect(303, "/login");
    if (!isDizEnabled())
      return fail(403, { officialFlowError: "Il flusso DIZ è disattivato in questo ambiente." });
  ...
  reimportDiz: async ({ locals, params, request }) => {
    if (!locals.ownerId) redirect(303, "/login");
    if (!isDizEnabled())
      return fail(403, { officialFlowError: "Il flusso DIZ è disattivato in questo ambiente." });
    const formData = await request.formData();
  ...
  resolveDiz: async ({ locals, params, request }) => {
    if (!locals.ownerId) redirect(303, "/login");
    const formData = await request.formData();
    const declarationId = String(formData.get("declarationId") ?? "");
    const roundTripId = String(formData.get("roundTripId") ?? "");
    ...
    for (const [key, value] of formData.entries()) {
      ...
      if (value === "current" || value === "official")
        choices[key.slice("conflict:".length)] = value;
    }
    try {
  ...
  addOfficialArtifact: async ({ locals, params, request }) => {
    if (!locals.ownerId) redirect(303, "/login");
    const formData = await request.formData();
    const declarationId = String(formData.get("declarationId") ?? "");
    ...
    try {
      ...
      if (
        ...
        flowError.message === "SECOND_RECEIPT_REGISTRATION_REQUIRED"
      )
        return fail(400, {
          officialFlowError:
  ...
  confirmPresentation: async ({ locals, params, request }) => {
    if (!locals.ownerId) redirect(303, "/login");
    const formData = await request.formData();
    const declarationId = String(formData.get("declarationId") ?? "");
    try {
  ...
  overrideOfficialStage: async ({ locals, params, request }) => {
    if (!locals.ownerId) redirect(303, "/login");
    const formData = await request.formData();
    const declarationId = String(formData.get("declarationId") ?? "");
    try {
