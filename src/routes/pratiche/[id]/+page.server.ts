...
} from "$lib/server/domain-calculations";
import {
  confirmDevolutionScenario,
  listDevolutionScenarios,
  saveDevolutionScenario,
} from "$lib/server/domain-devolution";
import { listCanonicalOccurrenceIds, saveCanonicalField } from "$lib/server/domain-fields";
import { synchronizeChecklist, updateChecklistItem } from "$lib/server/domain-checklist";
import { createSharedAsset, listSharedAssets } from "$lib/server/domain-assets";
import {
...
export const load: PageServerLoad = async ({ locals, params, url }) => {
  ...
  const operationalArea = isOperationalSectionId(requestedSection)
    ...
    : null;
  const operationalFields = operationalArea
    ? listOperationalAreaFields(operationalArea).map((field) =>
        createPracticeFieldView(field, field.operationalParity),
      )
    : [];
