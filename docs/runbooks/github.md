...
- release: riuso exact-tree della suite PR, benchmark, browser matrix e immagine ARM64 verificata.
...
Il workflow manuale `Release candidate` accetta soltanto il commit corrente di `main`, identifica l'unica pull request squash che lo ha prodotto, verifica che l'albero coincida con quello dell'HEAD e che tutti i gate richiesti della PR siano verdi. Questa equivalenza consente di riusare la suite pubblica già eseguita. Benchmark, scansione dipendenze, Chromium e WebKit restano obbligatori; i due browser girano in parallelo e in processi isolati dentro l'immagine Playwright fissata per digest. Un solo job ARM64 costruisce e prova l'immagine, crea un tar esclusivamente temporaneo per OSV-Scanner, lo elimina, pubblica su GHCR e rilegge l'immagine per digest. Come artefatto GitHub viene caricato soltanto il piccolo manifest con versione, commit, tree, piattaforma e riferimento immutabile. La scansione fallisce chiusa su errori o output non interpretabili e blocca qualunque vulnerabilità con un fix distribuibile. Le vulnerabilità senza fix, incluse quelle Critical, restano advisory accettate dal proprietario perché non esiste una correzione distribuibile da applicare; conteggi, severità e identificativi restano visibili fino all'aggiornamento della base o del pacchetto interessato. Scanner, immagine Playwright e azioni sono identificati da digest o SHA immutabili. La pull request verifica automaticamente le fonti ministeriali quando cambiano originali, catalogo o regole; i gate privati su corpus reale, software ufficiale, backup e VPS restano separati e obbligatori quando pertinenti.

## Production

Il workflow manuale `Production` accetta il commit completo di `main` e il run ID della relativa candidata. Prima di trasferire file verifica tramite API che quel run appartenga al workflow `Release candidate`, sia concluso con successo su `main` e abbia lo stesso SHA; scarica quindi il manifest exact-run, ne rilegge commit, tree, piattaforma e digest e registra un GitHub Deployment con task `sequent-production`. L'environment protetto deve contenere anche `SEQUENT_GHCR_USERNAME` e `SEQUENT_GHCR_TOKEN`, con permesso minimo di lettura del package; la credenziale viene trasmessa via stdin a una configurazione Docker root-only in `/run`, eliminata subito dopo il pull e rimossa nuovamente dal finalizzatore in caso di interruzione.

Il deploy acquisisce il lock Docker condiviso, richiede il runtime precedente healthy, esegue il pull del digest GHCR prima della manutenzione, richiede un database coerente senza job attivi, prova le migrazioni su una copia isolata e crea uno snapshot prima di sostituire il container Sequent. Durante lo switch un marker applicativo lascia disponibili le sole richieste non mutanti e risponde `503` alle altre. Il readback HTTPS avviene mentre il marker è ancora attivo e richiede un health pubblico esclusivamente generico; image ID e commit esatti vengono verificati separatamente sul runtime della VPS, insieme a utente non-root, filesystem in sola lettura, capability e controlli SQLite. Un errore ripristina automaticamente dati, configurazione e immagine precedenti. Solo dopo tutti i controlli il marker viene rimosso. Manifest, riferimento GHCR, ricevuta e image ID restano nel registro release della VPS con retention limitata al runtime e al rollback più recenti. La pulizia Docker ordinaria è eseguita dal timer dopo il deploy e non prolunga il percorso critico di Production.
