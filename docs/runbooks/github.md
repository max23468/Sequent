...
Il deploy acquisisce il lock Docker condiviso, richiede il runtime precedente healthy e un database coerente senza job attivi, prova le migrazioni su una copia isolata e crea uno snapshot prima di sostituire il container Sequent. Il readback richiede health pubblico, image ID e commit esatti, utente non-root, filesystem in sola lettura e controlli SQLite; un errore dopo l'arresto ripristina automaticamente dati, configurazione e immagine precedenti. Archivio, manifest, ricevuta e image ID restano nel registro release della VPS.
