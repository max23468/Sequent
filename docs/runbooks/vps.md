...
Il runtime applicativo resta inattivo finché non viene approvata una release. L'utente di sistema `sequent-runtime` non possiede login né home e riserva il confine dei dati operativi. `deploy/compose.example.yml` descrive i servizi isolati, ma non è una configurazione attiva e non autorizza il deploy.
...
La modifica atomica delle due flag si esegue senza riavviare il servizio e senza stampare gli altri valori privati:
...
Il `tmpfs` di `/tmp` usa gli stessi UID e GID numerici del processo applicativo e la modalità sticky standard `1777`. OCR, LibreOffice e gli altri processi figli possono così creare temporanei senza rendere scrivibile il filesystem dell'immagine. Il runner Codex riceve separatamente il solo workspace effimero condiviso sotto `/opt/sequent/tmp/codex-runtime`.

Il processo web Node resta non root, perde tutte le capability, usa i profili seccomp e AppArmor predefiniti e applica `no-new-privileges`; il filesystem dell'immagine resta in sola lettura. L'immagine contiene `/usr/bin/bwrap` setuid esclusivamente per il runner, ma `no-new-privileges` ne impedisce l'elevazione nel container web. Quando la flag Codex è attiva, Compose aggiunge un runner non esposto e senza mount operativi: soltanto quel container riceve le capability di namespace e la rimozione dei profili seccomp/AppArmor esterni richieste da Bubblewrap. `NET_ADMIN` appartiene a questo set minimo perché Bubblewrap la usa per creare il namespace di rete interno; `NET_RAW` resta assente. Il sandbox interno Codex resta obbligatorio, con rete degli strumenti di analisi disabilitata e filesystem limitato al workspace; il runner non riceve socket Docker, rete proxy o dati Hub Fatture. Con la flag spenta il runner non viene creato. L'attivazione richiede una nuova qualifica di `TG-CODEX` e un deploy deliberato.

La home Codex di Sequent risiede sotto `/opt/sequent/private/codex`, è montata soltanto nel runner e contiene esclusivamente autenticazione ChatGPT e stato creato dalla CLI dedicata. Non si riusa la home amministrativa generale e non si copiano `config.toml`, `requirements.toml` o `plugins`: la presenza di uno di questi elementi blocca la run. Il container web scambia con il runner soltanto workspace effimeri sotto `/opt/sequent/tmp/codex-runtime` e non può eseguire direttamente la CLI. La procedura completa è nel [runbook di qualificazione](codex-diz-qualification.md).

## Prima attivazione e deploy

La corsia `Production` trasferisce sulla VPS esclusivamente l'immagine ARM64 e il manifest prodotti dal run `Release candidate` exact-commit. Non esegue build sulla VPS. Nei deploy successivi alla prima attivazione, prima di creare il Deployment GitHub, invoca la manutenzione selettiva installata root-owned con età minima zero: l'assenza dello script, un layout non fidato, un lock occupato o una pulizia fallita bloccano il tentativo prima di qualsiasi mutazione del runtime. La manutenzione conserva sempre runtime corrente, rollback, immagini referenziate da container e identificativi trattenuti; il lock Docker condiviso impedisce che si sovrapponga a build, deploy o manutenzioni di Hub Fatture. La soglia disco del deploy misura quindi il margine realmente disponibile dopo avere rimosso soltanto immagini Sequent non più protette e continua a bloccare spazio davvero insufficiente prima dello switch.

Dopo il readback riuscito, la corsia ripete la stessa manutenzione con età minima zero: in quel momento l'immagine appena distribuita è il runtime corrente, quella sostituita è il rollback e soltanto le generazioni precedenti possono essere eliminate. Un errore in questa seconda pulizia non invalida la release già verificata e produce un warning non bloccante; il timer giornaliero ritenterà. L'immagine attiva non viene rimossa perché serve a riavvio, ricreazione e readback del container.

La corsia persiste sul runner l'identificativo del Deployment GitHub appena creato. Il finalizzatore eseguito anche dopo un annullamento chiude come fallito un Deployment ancora pendente, elimina il trasferimento temporaneo e, soltanto quando il job risulta annullato, ritenta per una finestra limitata la stessa manutenzione selettiva con età minima zero. Ogni tentativo deve acquisire il lock Docker condiviso: la pulizia può quindi iniziare solo dopo che il deploy interrotto ha completato il proprio rollback e rilasciato il lock. Se la riconciliazione o la manutenzione non riescono, il workflow emette un warning e il timer giornaliero conserva il ruolo di recupero; runtime corrente, rollback, container e immagini trattenute restano protetti.

Prima di ogni invocazione la corsia trasferisce anche `scripts/vps/run-trusted-deploy.sh`: ne copia i byte in un file temporaneo root-owned, verifica da root lo SHA-256 calcolato sull'HEAD esatto dal runner GitHub e solo allora lo installa come `/usr/local/sbin/sequent-run-trusted-deploy`, `root:root:0755`. Il launcher richiede il layout corrente root-owned della radice, di `releases` e di `snapshots`; proprietari o modalità diversi bloccano l'operazione. Verifica quindi l'HEAD e la pulizia del checkout, poi crea se assente l'account di sistema senza login `sequent-deploy`, con gruppo primario `ubuntu`: questo account può leggere gli oggetti del checkout ma non può modificare né il checkout né la directory temporanea Git che possiede in modo esclusivo. Da tale object database, con configurazioni globali e di sistema escluse e replace ref disabilitati, produce `git archive`, ricostruisce il tree estratto e lo confronta con il tree dell'esatto commit prima che root esegua alcuno script. Infine copia nel tree root-owned gli artefatti e ne confronta gli SHA-256 calcolati dal runner GitHub prima del trasferimento; il deploy usa soltanto queste copie immutabili. Il file di deploy deve conservare nel commit il modo Git `100755` ed essere `root:root:0755` nel tree verificato; viene poi interpretato con `/bin/bash`, così la corsia resta compatibile con `/run` montato `noexec` su Debian 13 senza allentare i controlli di provenienza. Nessuno script o artefatto scrivibile dall'account SSH viene elevato direttamente. L'invocazione operativa usa percorsi temporanei controllati:
...
Il comando serializza l'operazione sul lock Docker condiviso, verifica disco, HEAD e artefatto, controlla il database e l'assenza di job attivi, quindi avvia la candidata su una copia isolata dei dati collocata sotto il parent `snapshots` non sostituibile dall’account SSH. Prima del caricamento rifiuta un tag candidato referenziato da container e rimuove l'eventuale tag stale; l'archivio vincolato dallo SHA-256 crea quindi il nuovo tag. L'image ID registrato dal runner nel manifest resta evidenza del builder, mentre l'image ID runtime viene riletto dal tag appena caricato: i due possono differire tra image store containerd e Docker classico senza che differiscano i byte dell'archivio. Prima di interrompere il servizio richiede che l'eventuale directory della release precedente sia già nel layout corrente `root:root:0750`; qualunque altro layout blocca l'operazione. Soltanto dopo architettura ARM64, label OCI exact-commit, health e coerenza SQLite della copia attiva la manutenzione, arresta il container Sequent e crea uno snapshot esclusivo dei dati ormai quiescenti prima di applicare il nuovo Compose. Le operazioni Git sul checkout vengono eseguite senza privilegi come `ubuntu`; sia il Compose candidato sia quello di rollback provengono da tree Git immutabili e vengono consumati da file `root:root` sotto `/run`. Un marker nei dati operativi blocca con `503` le richieste mutanti e impedisce al job runner di reclamare attività durante l’intera transazione. Il readback HTTPS richiede il solo stato pubblico generico: lo stato diventa degradato anche sotto 5 GiB liberi o dal 90% di utilizzo del filesystem dei dati. Image ID runtime, label commit OCI, utente, filesystem, capability e database vengono verificati separatamente dalla VPS mentre il marker è ancora attivo; se un controllo fallisce vengono ripristinati automaticamente snapshot e immagine precedenti, altrimenti il marker viene rimosso come ultima operazione della transazione. Non esegue down migration e non modifica Caddy, Dynu, firewall o servizi estranei.
