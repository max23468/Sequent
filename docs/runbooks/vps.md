...
Il processo Node resta non root, perde tutte le capability, usa i profili seccomp e AppArmor predefiniti e applica `no-new-privileges`. Il filesystem dell'immagine resta in sola lettura e non contiene file setuid o setgid. Questo è il profilo Production qualificato finché Codex è spento. Il profilo Codex qualificato mantiene gli stessi confini Linux e aggiunge soltanto la flag deliberata, la home dedicata `/var/lib/sequent/.codex-sequent` e le restrizioni applicative del sandbox: non disabilita seccomp, AppArmor o sandbox e non aggiunge capability. L'attivazione richiede comunque una nuova qualifica di `TG-CODEX` e un deploy deliberato. OCR, LibreOffice e gli altri converter continuano a lavorare nel limite di CPU, memoria, processi e `tmpfs` già dichiarato.

La home Codex di Sequent contiene esclusivamente autenticazione ChatGPT e stato creato dalla CLI dedicata. Non si riusa la home amministrativa generale e non si copiano `config.toml`, `requirements.toml` o `plugins`: la presenza di uno di questi elementi blocca la run. La procedura completa è nel [runbook di qualificazione](codex-diz-qualification.md).
