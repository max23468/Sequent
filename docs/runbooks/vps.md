...
La corsia `Production` trasferisce sulla VPS esclusivamente l'archivio ARM64 e il manifest prodotti dal run `Release candidate` exact-commit. Non esegue build sulla VPS. La soglia disco del deploy viene verificata senza affidarsi a una pulizia preventiva: spazio realmente insufficiente blocca l'operazione prima dello switch. Soltanto dopo che distribuzione e readback sono riusciti, la corsia avvia la manutenzione selettiva root-owned con età minima zero. La manutenzione conserva sempre runtime corrente, rollback, immagini referenziate da container e identificativi trattenuti; il lock Docker condiviso impedisce che si sovrapponga a build, deploy o manutenzioni di Hub Fatture. Se lo script non esiste o la pulizia fallisce, la release già verificata resta valida e il workflow emette un warning non bloccante: il timer giornaliero ritenterà la stessa manutenzione.
