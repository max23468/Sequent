...
La corsia `Production` trasferisce sulla VPS esclusivamente l'archivio ARM64 e il manifest prodotti dal run `Release candidate` exact-commit. Non esegue build sulla VPS. Dopo avere verificato e allineato il checkout exact-commit, ma prima di creare la directory di arrivo e trasferire l'archivio, esegue la manutenzione selettiva root-owned già installata con età minima zero. La manutenzione conserva sempre runtime corrente, rollback, immagini referenziate da container e identificativi trattenuti; il lock Docker condiviso impedisce che si sovrapponga a build, deploy o manutenzioni di Hub Fatture. Se lo script non esiste ancora durante la prima attivazione, il passaggio viene saltato. La soglia disco del deploy resta invariata e viene verificata dopo il trasferimento, così spazio recuperabile non provoca un fallimento evitabile e spazio realmente insufficiente continua a bloccare l'operazione.
