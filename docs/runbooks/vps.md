...
Il processo Node resta non root, perde tutte le capability, usa i profili seccomp e AppArmor predefiniti e applica `no-new-privileges`. Il filesystem dell'immagine resta in sola lettura e non contiene file setuid o setgid. Questo è il profilo Production qualificato finché Codex è spento: l'eventuale futura attivazione di Codex richiede un profilo runtime separato, una nuova qualifica di `TG-CODEX` e un deploy deliberato; non si conservano preventivamente capability o sandbox disabilitate nel servizio pubblico. OCR, LibreOffice e gli altri converter continuano a lavorare nel limite di CPU, memoria, processi e `tmpfs` già dichiarato.
